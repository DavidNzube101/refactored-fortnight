...
export function ChatScreen() {
  ...
  const userStatusRef = firestore().collection("users").doc(chatWithUserId);

  useEffect(() => {
    const unsubscribeStatus = userStatusRef.onSnapshot((doc) => {
      if (doc.exists) {
        const userData = doc.data();
        setUserStatus(userData?.isOnline ? "Online" : "Offline");
        setProfileImage(
        ...
        <View style={[external.mh_10, external.fg_1]}>
          ...
          <Text
            style={[
              ...
              external.mt_2,
              { color: appColors.buttonBg, textAlign: textRTLStyle },
            ]}
          >

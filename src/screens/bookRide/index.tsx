...
export function BookRide() {
  ...
  const ZoneArea = zoneValue?.data?.[0]?.locations || null;
  const { driverData } = useSelector((state) => state.allDriver);
  const { vehicleTypedata = null } = useSelector(
  ...
  const minWeightCharge =
    selectedVehicleData?.min_per_weight_charge * distance || null;
  const maxWeightCharge =
    selectedVehicleData?.max_per_weight_charge * distance || null;
  ...
  const ParcelRecommendCharge = (
    (parseFloat(ParcelMinCharge) + parseFloat(ParcelMaxCharge)) /
    2
  ).toFixed(2);
...
`;
  ...
  const paymentData = (index) => {
    ...
    setSelectedOption(activePaymentMethods[index].name);
  };
  ...
  const BookRideRequest = async (forme) => {
    ...
    try {
      ...
      if (forme.selectedImage) {
        formData.append("cargo_image_id", {
  ...
  const handleBookRide = () => {
    if (categoryOption == "Parcel") {
      if (fareValue < ParcelMinCharge) {
        setWarning(true);
        setWarningMessage(
          `Minimum fare should be ${Number(ParcelMinCharge).toFixed(2)}`
        );
      } else if (fareValue > ParcelMaxCharge) {
        setWarning(true);
        setWarningMessage(
          `Maximum fare should be ${Number(ParcelMaxCharge).toFixed(2)}`
        );
      } else {
        setWarning(false);
    ...
    } else if (categoryOption === "Freight") {
      if (fareValue < minChargeRide) {
        setWarning(true);
        setWarningMessage(`Minimum fare should be ${minChargeRide.toFixed(2)}`);
      } else if (fareValue > maxChargeRide) {
        setWarning(true);
        setWarningMessage(`Maximum fare should be ${maxChargeRide.toFixed(2)}`);
      } else {
        setWarning(false);
        setIsExpanding(!isExpanding);
        if (!isExpanding) {
  ...
  const renderItem1 = ({ item, index }) => (
    <>
      ...
      {index !== 3 ? <View style={[styles.borderPayment,{borderColor:colors.border}]} /> : null}
    </>
  );
              ...
              >
                ...
                {filteredDrivers?.map((driver, index) => (
                  ...
                  >
                    <CustomMarker
                      imageUrl={
                        selectedVehicleData?.vehicle_map_icon?.original_url
                      }
                    />
                  </Marker>
                ))}
          ...
          >
            {selectedOption === null ? (
              <>
                ...
                />
                <View style={[external.mh_10, external.mb_10]}>
                  <Text
                    style={[
                      styles.title,
                ...
                >
                  {parcelWeight ? (
                    ...
                    >
                    {minChargeRide.toFixed(1)} - {maxChargeRide.toFixed(1)}
                      <Text
                        style={[styles.priceTitle, { color: textColorStyle }]}
                      >
                        {" "}
              ...
              <>
                <View style={styles.switchContainer}>
                  ...
                  <View style={[external.mt_20]}>
                    ...
                    {/* <TouchableOpacity style={[external.fd_row, external.js_space]} onPress={chooseRider}>
                      <View
                        style={[
                          external.fd_row,
                          external.ai_center,
                    ...
                    <TouchableOpacity
                      style={[external.fd_row, external.js_space, { flexDirection: viewRTLStyle }]}
                      onPress={chooseRider}
                    >
                      <View style={[external.fd_row, external.ai_center]}>
                        <NewContact />
                        <Text style={[styles.chooseAnotherAccount, { marginLeft: 8 }]}>
                          {translateData.contact}
                        </Text>
                      </View>
                      <Forword />
                  ...
                  >
                    {/* <Button
                      ...
                      onPress={handleSkip}
                    /> */}
                    {/* <Button width={'47%'} title={'Continue'} onPress={chooseRider} /> */}
                  </View>
                </View>
              </>
            ) : null}
            ...
            >
              <TouchableOpacity
                ...
                style={[styles.switchUser, {
                  ...
                  ...(selectedOption ? { top: windowHeight(7) } : {}),
                }]}
              >
            ...
            {/* <View style={{flexDirection:'row', marginHorizontal:windowWidth(15), justifyContent:'space-between'}}>
              <View style={{backgroundColor:appColors.lightButton, height:windowHeight(28), width:windowWidth(200), borderRadius:windowHeight(20), justifyContent:'center', alignItems:'center', flexDirection:'row', }}>
                <View style={{marginHorizontal:windowWidth(8)}}>
                  {data[0].icon}
                </View>
                <Text style={{color:appColors.buttonBg, fontFamily:appFonts.regular,marginHorizontal:windowWidth(8)}}>{data[0].label}</Text>
              </View>
              <View style={{backgroundColor:appColors.lightButton, height:windowHeight(28), width:windowWidth(200), borderRadius:windowHeight(20), justifyContent:'center', alignItems:'center', flexDirection:'row', }}>
                <View style={{marginHorizontal:windowWidth(8)}}>
                  {data[2].icon}
                </View>
                <Text style={{color:appColors.buttonBg, fontFamily:appFonts.regular,marginHorizontal:windowWidth(8)}}>{data[2].label}</Text>
              </View>
            </View> */}
            {/* <KmDetails onPress={openSheet} onPackageSelect={handleScreenSelection}/> */}
            {!RideBooked ? (

...
import { windowWidth, appFonts } from '@src/themes';
import { useSelector } from 'react-redux';

export function CompleteRide() {
  const { textColorStyle, linearColorStyle, textRTLStyle, viewRTLStyle, bgContainer, t } = useValues();
    ...
    <Header
      ...
      container={
        <ScrollView
          ...
          <RideDetails />
          <TouchableOpacity onPress={handlePreeCancel} style={styles.helpView}>
          ...
          >
            <View style={styles.modalBg}>
              <View style={[styles.modalBgMain, { backgroundColor: linearColorStyle }]}>
                <TouchableOpacity style={styles.close} onPress={() => setModalCancelVisible(false)}>
                  <Close />
                </TouchableOpacity>
                <Text style={[styles.cancelTitle, { color: textColorStyle }]}>{translateData.whyCancel}</Text>
                {cancelReason?.map(item => (
                  <TouchableOpacity key={item.id} style={[styles.container2, { backgroundColor: bgContainer, flexDirection: viewRTLStyle }]} onPress={gotoHelp}>
                ...
                <Button backgroundColor={appColors.buttonBg} width={300} title={translateData.close} onPress={() => setModalCancelVisible(false)}
                />
              </View>
            </View>
          </Modal>

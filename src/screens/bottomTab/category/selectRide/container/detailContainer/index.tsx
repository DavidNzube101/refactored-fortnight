...
export function DetailContainer() {
  ...
  const { pickupLocation, categoryId, zoneValue } = route.params;
  ...
  const ZoneArea = zoneValue?.data?.[0]?.locations || [
    { lat: 21.2278069992587, lng: 72.84657310054922 },
  ];
  ...
  const filteredVehicle = packageVehicle?.find(
    (vehicle) => vehicle?.id === selectedItem
  );

  const packageTotalMinutes = selectedPackageDetails?.hour * 60;
  const packageTotalDistance = selectedPackageDetails.distance;
  ...
  const recommendedPackageCharge = (minPackageCharge + maxPackageCharge) / 2;
  ...
  const handleBookRide = () => {
    if (fareValue < minPackageCharge) {
      setWarning(true);
      setWarningMessage(
        `Minimum fare should be ${minPackageCharge.toFixed(2)}`
      );
    } else if (fareValue > maxPackageCharge) {
      setWarning(true);
      setWarningMessage(
        `Maximum fare should be ${maxPackageCharge.toFixed(2)}`
      );
    } else {
      setWarning(false);
  ...
  const renderItem1 = ({ item, index }) => (
    <TouchableOpacity onPress={() => paymentData(index)}>
      <View
        style={[
